;;; monokai-y-theme.el --- A fruity color theme for Emacs.

;; Copyright (C) 2011-2013

;; Author: Kelvin Smith <oneKelvinSmith@gmail.com>
;; URL: http://github.com/oneKelvinSmith/monokai-y-emacs
;; Version: 0.2.0
...
;;; Commentary:
;;
;; A port of the popular Textmate theme Monokai-Y for Emacs 24, built on top
;; of the new built-in theme support in Emacs 24.
;;
;;; Credits:
;;
;; Wimer Hazenberg created the original theme.
;; - http://www.monokai-y.nl/blog/2006/07/15/textmate-color-theme/
;;
...
;;
;; K. Adam Christensen for his personal monokai-y theme that addresses 256 colours.
;; - https://github.com/pope/personal/blob/master/etc/emacs.d/monokai-y-theme.el
;;
...
(unless (>= emacs-major-version 24)
  (error "The monokai-y theme requires Emacs 24 or later!"))

(deftheme monokai-y "The monokai colour theme")

(defgroup monokai-y nil
  "Monokai-Y theme options.
The theme has to be reloaded after changing anything in this group."
  :group 'faces)

(defcustom monokai-y-distinct-fringe-background nil
  "Make the fringe background different from the normal background color.
Also affects 'linum-mode' background."
  :type 'boolean
  :group 'monokai-y)

(defcustom monokai-y-use-variable-pitch t   ; default "t", changed by oracleyue
  "Use variable pitch face for some headings and titles."
  :type 'boolean
  :group 'monokai-y)

(defcustom monokai-y-high-contrast-mode-line nil
  "Make the active/inactive mode line stand out more."
  :type 'boolean
  :group 'monokai-y)

(defcustom monokai-y-height-minus-1 0.8
  "Font size -1."
  :type 'number
  :group 'monokai-y)

(defcustom monokai-y-height-plus-1 1.1
  "Font size +1."
  :type 'number
  :group 'monokai-y)

(defcustom monokai-y-height-plus-2 1.15
  "Font size +2."
  :type 'number
  :group 'monokai-y)

(defcustom monokai-y-height-plus-3 1.2
  "Font size +3."
  :type 'number
  :group 'monokai-y)

(defcustom monokai-y-height-plus-4 1.3
  "Font size +4."
  :type 'number
  :group 'monokai-y)
...
(defun create-monokai-y-theme (&optional frame)
  "Create the monokai-y theme.
Takes and optional `FRAME' as reference."
  (let* ((class '((class color) (min-colors 89)))
         ...
         ;; Adaptive colors
         (monokai-y-fg       (if (in-terminal) "#F5F5F5" "#F8F8F2"))
         (monokai-y-bg       (if (in-terminal) "#1B1E1C" "#272822"))  ; oracleyue: modified
         ;; (monokai-y-bg       (if (in-terminal) "#1B1E1C" "#272822"))
         (monokai-y-line-num (if (in-terminal) "#8F908A" "#8F908A"))  ; oracleyue: added
         (monokai-y-hl-line  (if (in-terminal) "#212121" "#3E3D31"))
         ;(monokai-y-hl-line-y   (if (in-terminal) "#212121" "#585858"))  ; oracleyue: added, to change mode-line color, "s-mode-line-bg"
         (monokai-y-hl       (if (in-terminal) "#303030" "#49483E"))
         (monokai-y-emph     (if (in-terminal) "#FFFAFA" "#F8F8F0"))
         (monokai-y-comments (if (in-terminal) "#8B8878" "#75715E"))
         (region-bg         (if (in-terminal) "#005F87" "#005F87"))  ; oracleyue: added

         ;; Adaptive higher/lower contrast accented colors
         (monokai-y-fg-hc    (if (in-terminal) "#171A0B" "#141414"))
         (monokai-y-fg-lc    (if (in-terminal) "#141414" "#171A0B"))
         ...
         (s-variable-pitch (if monokai-y-use-variable-pitch
                               'variable-pitch 'default))
         (s-fringe-bg (if monokai-y-distinct-fringe-background
                          monokai-y-hl monokai-y-bg))

         (s-mode-line-fg (if monokai-y-high-contrast-mode-line
                             monokai-y-bg monokai-y-fg))
         (s-mode-line-bg (if monokai-y-high-contrast-mode-line
                             monokai-y-fg monokai-y-hl-line))
                             ;monokai-y-fg monokai-y-hl-line-y))  ; oracleyue: modified
         (s-mode-line-buffer-id-fg (if monokai-y-high-contrast-mode-line
                                       'unspecified green))
         (s-mode-line-inactive-fg (if monokai-y-high-contrast-mode-line
                                      monokai-y-fg monokai-y-comments))
         (s-mode-line-inactive-bg (if monokai-y-high-contrast-mode-line
                                      monokai-y-hl-line monokai-y-bg))
         (s-mode-line-inactive-bc (if monokai-y-high-contrast-mode-line
                                      monokai-y-fg monokai-y-hl-line)))
    (custom-theme-set-faces
     ...
     ;; basic coloring
     `(default
        ((,class (:foreground ,monokai-y-fg
                              :background ,monokai-y-bg))))

     `(shadow
       ((,class (:foreground ,monokai-y-comments))))

     `(match
       ((,class (:background ,monokai-y-hl
                             :foreground ,monokai-y-emph
                             :weight bold))))

     `(cursor
       ((,class (:foreground ,monokai-y-bg
                             :background ,monokai-y-fg
                             :inverse-video t))))
     ...
     `(cua-rectangle-noselect
       ((,class (:inherit region
                          :background ,monokai-y-hl
                          :foreground ,monokai-y-comments))))
     ...
     `(ecb-tree-guide-line-face
       ((,class (:inherit ecb-default-general-face
                          :foreground ,monokai-y-hl
                          :height 1.0))))

     ;; ee
     `(ee-bookmarked
       ((,class (:foreground ,monokai-y-emph))))
     ...
     ;;   ((,class (:foreground ,monokai-y-comments))))
     `(font-lock-doc-face
       ((,class (:foreground "#65B042"))))

     ;; `(font-lock-function-name-face
     ;;   ((,class (:foreground ,green))))
     `(font-lock-function-name-face
       ((,class (:foreground "#F1266F"
                             :italic t))))
     ...
     ;;   ((,class (:foreground ,yellow))))
     `(font-lock-string-face
       ((,class (:foreground "#DFD874"))))

     ;; `(font-lock-type-face
     ;;   ((,class (:foreground ,blue
     ;;                         :italic t))))
     `(font-lock-type-face
       ((,class (:foreground "#89BDFF"
                             :underline t))))
     ...
     `(ace-jump-face-foreground
       ((,class (:foreground ,red :background ,monokai-y-bg
                             :inverse-video nil
                             :weight bold))))
     ...
     `(font-latex-italic-face
       ((,class (:inherit italic :foreground ,monokai-y-emph))))
     ...
     `(font-latex-sectioning-0-face
       ((,class (:inherit font-latex-sectioning-1-face
                          :height ,monokai-y-height-plus-1))))
     ...
     `(font-latex-slide-title-face
       ((,class (:inherit (,s-variable-pitch font-lock-type-face)
                          :weight bold
                          :height ,monokai-y-height-plus-3))))

     `(font-latex-string-face
       ((,class (:foreground ,cyan))))

     `(font-latex-subscript-face
       ((,class (:height ,monokai-y-height-minus-1))))
     ...
     `(cfw:face-title
       ((,class (:inherit ,s-variable-pitch
                          :foreground ,yellow
                          :weight bold :height ,monokai-y-height-plus-4))))
     ...
     `(ediff-even-diff-A
       ((,class (:background ,monokai-y-comments
                             :foreground ,monokai-y-fg-lc ))))

     `(ediff-odd-diff-A
       ((,class (:background ,monokai-y-comments
                             :foreground ,monokai-y-fg-hc ))))
     ...
     `(helm-grep-match                ; used for find-file-grep & occur
       ...
       ((,class (:foreground ,magenta
                             :background ,monokai-y-bg))))      ;oracleyue, modified
     ...
     `(helm-selection
       ((,class (:background ,region-bg        ;oracleyue, default "monokai-y-hl"
     ...
     `(helm-swoop-target-word-face
       ((,class (:foreground ,red    ;oracleyue, modified; default "magenta"
                             ;:background unspecified))))
                             :background ,monokai-y-bg))))    ;oracleyue, added
     ...
     ;; hl-line-mode
     `(hl-line
       ((,class (:background ,monokai-y-hl-line
     ...
     ;; linum-mode
     `(linum
       ((,class (:foreground ,monokai-y-line-num
                 ;:foreground ,monokai-y-comments
                             :background ,s-fringe-bg))))
     ...
     `(magit-log-head-label-default
       ((,class (:background ,monokai-y-hl :box 1))))
     ...
     `(markdown-header-face-1
       ((,class (:inherit markdown-header-face
                          :height ,monokai-y-height-plus-4))))
     ...
     `(markdown-header-face-3
       ((,class (:inherit markdown-header-face
                          :height ,monokai-y-height-plus-2))))
     ...
     `(org-agenda-structure
       ((,class (:foreground ,monokai-y-emph
                             ...
                             :height ,monokai-y-height-plus-1
                             :underline nil
                             :box (:line-width 2 :color ,monokai-y-bg)))))
     ...
     `(org-agenda-date
       ((,class (:foreground ,monokai-y-comments
                             ...
                             :slant normal
                             :height 1.0
                             :box (:line-width 2 :color ,monokai-y-bg)))) t)
     ...
     ;; org-habit (clear=blue, ready=green, alert=yellow, overdue=red. future=lower contrast)   ; commented by the default author
     ...
     `(org-document-title
       ((,class (:inherit ,s-variable-pitch
                          :foreground ,monokai-y-emph
                          :weight bold
                          :height ,monokai-y-height-plus-1)))) ; oracleyue: 4
     ...
     `(popup-scroll-bar-foreground-face
       ((,class (:background ,monokai-y-emph))))
    ...
    (custom-theme-set-variables
     'monokai-y
     `(ansi-color-names-vector [,monokai-y-bg ,red ,green ,yellow
                                            ,blue ,magenta ,cyan ,monokai-y-fg])
     ...
     `(highlight-tail-colors
       '((,monokai-y-hl . 0)(,green-lc . 20)(,cyan-lc . 30)(,blue-lc . 50)
         (,yellow-lc . 60)(,orange-lc . 70)(,magenta-lc . 85)(,monokai-y-hl . 100)))
     ...
     ;; weechat
     `(weechat-color-list
       '(unspecified ,monokai-y-bg ,monokai-y-hl
                     ,red-d ,red
                     ,green-d ,green
                     ,yellow-d ,yellow
                     ,blue-d ,blue
...
(create-monokai-y-theme)

(add-hook 'server-visit-hook 'create-monokai-y-theme )

(provide-theme 'monokai-y)
