...
- =C-s=: use ~grep~ to match patterns in the current buffer;
- =C-u C-s=: use ~grep~ to match patterns any files with the same extension in the current folder;
...
The following are the settings for ~ack~ or ~rg~, if you prefer to them to replace ~grep~ or ~ag~:
#+BEGIN_SRC
